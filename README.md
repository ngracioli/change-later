...
- **`bulk-reader` is exempt from its own gate.** `PreToolUse` fires for subagent tool calls too, not just the main agent — without the `agent_type === "bulk-reader"` check in both hooks, the worker's own `Read` on the file it was dispatched to read would deny itself (deny loop). Confirmed against Claude Code docs (subagent hooks + available `PreToolUse` payload fields).
